...
import { Loader2, Building, MapPin, DollarSign, Bed, Bath, Square, Crown, TrendingUp, Link, Plus, X, FileText, Upload } from 'lucide-react';
...
export function PropertyForm({ initialData, onSubmit, onCancel, loading }: PropertyFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Set PDF preview if PDF URL exists
      if (initialData.pdf_url) {
        setPdfPreview(initialData.pdf_url);
      }
    }
  }, [initialData]);
  ...
  const uploadPdf = async (file: File): Promise<string> => {
    ...
    const filePath = `property-pdfs/${fileName}`;
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <Label htmlFor="pdf-upload" className="flex items-center gap-2">
                  ...
                  Property Brochure (PDF) - Limit 1 file
                </Label>
                ...
                <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-sm">
                            {uploadedPdf ? uploadedPdf.name : 'Property Brochure'}
                          </p>
                          <p className="text-xs text-gray-500">
                            {uploadedPdf ? `Size: ${(uploadedPdf.size / 1024 / 1024).toFixed(2)} MB` : 'PDF Brochure'}
                          </p>
                        </div>
                        <Button
                          type="button"
                  ...
                  <p className="text-xs text-muted-foreground">
                    Upload a single PDF brochure for this property. Maximum file size: 10MB
                  </p>
                </div>
              </div>
            ...
            <div className="flex gap-3 pt-6 border-t">
              <Button type="submit" className="flex-1" disabled={loading || pdfUploading} size="lg">
